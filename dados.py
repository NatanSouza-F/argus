...
def obter_taxa_conversao():
    ...
    df = pd.read_sql(query, conexao)
    conexao.close()
    return (df.iloc[0]['com_consorcio'] / df.iloc[0]['total']) * 100 if df.iloc[0]['total'] > 0 else 0
...
def obter_distribuicao_uf(renda_min=10000):
    ...
    WHERE c.Renda_Mensal > {renda_min}
      AND c.UF IN ({','.join([f"'{uf}'" for uf in UFS_VALIDAS])})
    GROUP BY c.UF
...
def obter_heatmap_uf_produto():
    ...
    INNER JOIN Fato_Contratos f
        ON c.ID_Cliente = f.ID_Cliente
        AND f.Status_Contrato = 'Ativo'
    WHERE c.UF IN ({','.join([f"'{uf}'" for uf in UFS_VALIDAS])})
...
def obter_lista_ufs():
    ...
    SELECT DISTINCT UF 
    FROM Dim_Clientes
    WHERE UF IN ({','.join([f"'{uf}'" for uf in UFS_VALIDAS])})
    ORDER BY UF
    """
    df = pd.read_sql(query, conexao)
    conexao.close()
